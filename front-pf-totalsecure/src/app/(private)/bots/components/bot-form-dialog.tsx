  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (bot) {
        ...
        toast({ title: 'Bot actualizado', description: 'El bot ha sido actualizado correctamente' });
      } else {
        await createBot(formData);
        toast({ title: 'Bot creado', description: 'El bot ha sido creado correctamente' });
      }
      onSave();
      onOpenChange(false);
    } catch (error) {
      toast({ title: 'Error', description: 'Hubo un error al guardar el bot.', variant: 'destructive' });
    } finally {
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="alertType" name="alertType" value={formData.alertType} onChange={handleChange} className="col-span-3" required />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="channel" className="text-right">Canal</Label>
              <Select value={formData.channelId != null ? formData.channelId.toString() : 'none'} onValueChange={handleChannelChange}>
                <SelectTrigger className="col-span-3"><SelectValue placeholder="Seleccionar canal" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Sin canal</SelectItem>
                  {channels.map(channel => <SelectItem key={channel.id} value={channel.id.toString()}>{channel.name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
